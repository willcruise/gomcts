...
python3 -m gomcts.training.train_from_sgf \
    --sgf_dir ~/go_data \
    --board_size 19 \
    --epochs 15 \
    --batch_size 64 \
...
watch -n 10 'ls -lh ~/gomcts/artifacts/weights.pt'
...
```bash
# Weights saved to:
~/gomcts/artifacts/weights.pt

# Test your trained network:
python3 -m gomcts.apps.startgame --sims 64 --size 19

# Or continue with reinforcement learning:
python3 -m gomcts.training.trainwithkatago \
    --games 500 \
    --sims 64 \
    --size 19 \
    --lr 0.0003
...
### Option 1: Just Play
```bash
python3 -m gomcts.apps.startgame --sims 100 --size 19
```
...
```bash
python3 -m gomcts.training.selftraining \
    --games 500 \
    --sims 64 \
    --size 19 \
    --lr 0.0001
