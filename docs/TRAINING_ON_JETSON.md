...
python -m gomcts.training.trainwithkatago --config configs/train_jetson_vs_katago.yaml
...
python -m gomcts.training.trainwithkatago --games 200 --sims 64 --size 9

# Phase 2: Self-play to diversify (100 games, faster)
python -m gomcts.training.selftraining --games 100 --sims 64 --size 9
...
1. **Train on Cloud**: Use higher sims (1000+) and more games
   ```bash
   # On cloud GPU
   python -m gomcts.training.trainwithkatago --games 2000 --sims 200 --size 9
   ```

2. **Download Weights**: Copy `artifacts/weights.pt` to your Jetson

3. **Continue Training**: Fine-tune on Jetson
   ```bash
   # On Jetson - continue from cloud weights
   python -m gomcts.training.trainwithkatago --games 500 --sims 64 --size 9
   ```
...
Edit `gomcts/neural/policy.py` line 27:
```python
...
python -m gomcts.training.trainwithkatago --games 500 --sims 64 --size 5

# Phase 2: Medium board (7x7)
python -m gomcts.training.trainwithkatago --games 500 --sims 64 --size 7

# Phase 3: Standard (9x9)
python -m gomcts.training.trainwithkatago --games 1000 --sims 64 --size 9
```
...
python -m gomcts.training.trainwithkatago --games 10 --sims 64  # Watch win rate
```
...
### KataGo Not Found
```bash
python -m gomcts.training.trainwithkatago --auto_install_assets --games 100
```
