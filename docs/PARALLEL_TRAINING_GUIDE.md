...
The new `gomcts/training/train_from_sgf_parallel.py` maximizes hardware utilization through parallelization.
...
```bash
python3 -m gomcts.training.train_from_sgf_parallel \
    --sgf_dir ~/go_data \
    --board_size 19 \
    --epochs 15 \
    --batch_size 64
...
### Use Original (`gomcts/training/train_from_sgf.py`) if:
- ✅ Very small dataset (<100 games)
- ✅ Limited RAM (<2GB available)
- ✅ Single-core CPU
- ✅ Debugging (simpler code)

### Use Parallel (`gomcts/training/train_from_sgf_parallel.py`) if:
