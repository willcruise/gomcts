...
- **`gomcts/training/train_from_sgf.py`** - The training script itself
...
- **`gomcts/neural/policy.py`** - Neural network implementation (both heads)
- **`gomcts/training/train_from_sgf.py`** - Supervised learning from SGF files
- **`gomcts/training/trainwithkatago.py`** - Train against KataGo
- **`gomcts/training/selftraining.py`** - Pure self-play training

### Helper Scripts

- **`scripts/train_jetson.sh`** - One-command Jetson training
- **`scripts/download_sample_sgf.sh`** - SGF download helper
- **`examples/sample_sgf_games/`** - Test files to try immediately
...
→ Run: 
```bash
python -m gomcts.training.train_from_sgf --sgf_dir ./examples/sample_sgf_games --epochs 3 --board_size 9
```
...
2. Test with sample files: `python -m gomcts.training.train_from_sgf --sgf_dir ./examples/sample_sgf_games --epochs 3`
3. Play against it: `python -m gomcts.apps.startgame --sims 64`

### Intermediate:
1. Read `SGF_DOWNLOAD_TUTORIAL.md`
2. Download 500 SGF files
3. Train for real: `python -m gomcts.training.train_from_sgf --sgf_dir ~/go_games --epochs 10`
...
A: Yes! Use `./examples/sample_sgf_games/` directory (already included)
...
# Save to ~/go_games/kgs_9x9/
# Then:
python -m gomcts.training.train_from_sgf --sgf_dir ~/go_games/kgs_9x9 --board_size 9 --epochs 10
```
