...
from gomcts.mcts_wrappers import build_mcts_standard
from gomcts.neural.policy import MLPPolicyValue


def _winner_simple(b: Board, komi: float = 0.0) -> int:
    """Return winner from Black's perspective: +1 black, -1 white, 0 draw."""
    margin = float(
        rules.final_margin(
            b.grid,
...
def _index_to_rc(b: Board, a: int) -> tuple:
...
def cli(size: int = 9, default_sims: int = 50, default_temp: float = 1.0) -> None:
    net = MLPPolicyValue()
    board = Board(int(size), enforce_rules=True, forbid_suicide=True, ko_rule='simple')

    print(
        "gomcts CLI ready. Commands: boardsize N | clearboard | showboard | "
        "genmove [b|w] [sims] [temp] | play [b|w] [coord|pass] | finalscore | undo | exit"
    )
    while True:
        try:
            line = input("> ").strip()
        ...
        if cmd == "exit":
            ...
            mcts = build_mcts_standard(net, size=int(getattr(board, "size", 9)))
            ...
            try:
                action = _parse_coord(move_str, int(getattr(board, "size", 9)))
            except ValueError:
            ...
            winner = _winner_simple(board, komi=float(getattr(board, "komi", 0.0)))
            if winner > 0:
