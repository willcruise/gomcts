...
from gomcts.core.board import Board9x9, Board
from gomcts.core.mcts import ScoreAwareMCTS, temperature_schedule
from gomcts.mcts_wrappers import build_mcts_standard
from gomcts.neural.policy import MLPPolicyValue, infer_policy_value

_REPO_ROOT = Path(__file__).resolve().parents[2]

def _weights_path() -> Path:
    return _REPO_ROOT / "artifacts" / "weights.pt"
...
def train_on_selfplay(net: MLPPolicyValue,
    ...
    eval_store = eval_dir if eval_dir is not None else str(_REPO_ROOT / "artifacts" / "eval_snapshots")
    ...
    for gi in range(games):
        ...
        if int(eval_every) > 0 and ((gi + 1) % int(eval_every) == 0):
            ...
            try:
                src = _weights_path()
                if src.exists():
                    shutil.copy2(str(src), cand_path)
            except Exception:
                pass
